 ...
 * 課題完了ユースケースの関数の型です。
 */
export type ExecuteCompleteTaskUseCase = (params: CompleteTaskParams) => Promise<void>;

/**
 * 課題完了ユースケースを作成します。
 *
 * @param dependencies 依存関係を指定します。
 * @returns 課題完了ユースケースを返します。
 */
export const createCompleteTaskUseCase = (
  dependencies: Dependencies,
): ExecuteCompleteTaskUseCase => {
  const { taskRepository, eventPublisher } = dependencies;

  /**
   * 課題の完了を扱うユースケースです。
   *
   * @param params 課題完了に必要なパラメータです。
   * @throws {ValidationError} 指定されたパラメータのいずれかが不正な場合にスローされます。
   * @throws {DomainError} 課題が存在しない場合、所有者でない場合、または完了できない状態の場合にスローされます。
   * @throws {InfrastructureError} 課題の取得・保存またはイベント発行に失敗した場合にスローされます。
   */
  const executeCompleteTaskUseCase = async (params: CompleteTaskParams) => {
  ...
  return executeCompleteTaskUseCase;
};
