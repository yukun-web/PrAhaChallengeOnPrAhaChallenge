...
import type { ExecuteRequestChangesUseCase } from "./request-changes.use-case";
import { createRequestChangesUseCase } from "./request-changes.use-case";

describe("課題修正依頼ユースケース", () => {
  /**
   ...
   * ユースケースの実行関数です。
   */
  let executeUseCase: ExecuteRequestChangesUseCase;

  beforeEach(() => {
    vi.clearAllMocks();
    executeUseCase = createRequestChangesUseCase({
      taskRepository: taskRepositoryMock,
      eventPublisher: eventPublisherMock,
    });
  });

  test("課題を取組中状態に戻して保存する", async () => {
  ...
  test("レビュー待ちでない課題の場合はドメインエラーを返し保存もイベント発行もしない", async () => {
    ...
    await expect(act).rejects.toBeInstanceOf(DomainError);
    await expect(act).rejects.toMatchObject({ code: "REQUEST_CHANGES_NOT_ALLOWED" });
    expect(taskRepositoryMock.save).not.toHaveBeenCalled();
    expect(eventPublisherMock.publish).not.toHaveBeenCalled();
  });
});
