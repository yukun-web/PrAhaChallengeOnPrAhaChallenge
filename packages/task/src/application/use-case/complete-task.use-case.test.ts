...
import type { ExecuteCompleteTaskUseCase } from "./complete-task.use-case";
import { createCompleteTaskUseCase } from "./complete-task.use-case";
...
describe("課題完了ユースケース", () => {
   ...
   * ユースケースの実行関数です。
   */
  let executeUseCase: ExecuteCompleteTaskUseCase;

  beforeEach(() => {
    vi.clearAllMocks();
    executeUseCase = createCompleteTaskUseCase({
      taskRepository: taskRepositoryMock,
      eventPublisher: eventPublisherMock,
    });
  });

  test("課題を完了状態にして保存する", async () => {
  ...
  test("レビュー待ちでない課題の場合はドメインエラーを返し保存もイベント発行もしない", async () => {
    ...
    await expect(act).rejects.toBeInstanceOf(DomainError);
    await expect(act).rejects.toMatchObject({ code: "COMPLETE_NOT_ALLOWED" });
    expect(taskRepositoryMock.save).not.toHaveBeenCalled();
    expect(eventPublisherMock.publish).not.toHaveBeenCalled();
  });
});
