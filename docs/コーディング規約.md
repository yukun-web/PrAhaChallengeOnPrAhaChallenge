...
- エンティティのテストデータは `domain/testing` のヘルパー関数を使用します（例: `createDummyParticipant`）。
  ...
  - ダミー生成関数は `Partial<ReconstructXxxParams>` を受け取り、内部で `Xxx.reconstruct` を使用します。
- `00000000-0000-0000-0000-000000000000` は NullObject パターン用に予約されています。テストデータには使用しないでください。
